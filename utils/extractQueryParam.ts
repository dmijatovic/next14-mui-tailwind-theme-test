...
export function decodeQueryParam({query,param,castToType='string',defaultValue}:{
  query: ParsedUrlQuery|null, param: string, castToType?: ('string' | 'number' | 'json-encoded'),
  defaultValue:any
}){
  try{
...
export function decodeSearchParam({query, param, castToType = 'string', defaultValue}: {
  query: ReadonlyURLSearchParams | null, param: string, castToType?: ('string' | 'number' | 'json-encoded'),
  defaultValue: any
}) {
  try {
    if (query !== null) {
      const rawVal = query.get(param)
      // if value is not "actionable" we return default value
      if (typeof rawVal == 'undefined' || rawVal === '' || rawVal === null) return defaultValue
      // if cast to type is not defined we return raw value
      if (typeof castToType === 'undefined') return rawVal
...
// REFACTORED AND MOVED to apiOrganisationParams
// export function ssrOrganisationParams(query: ReadonlyURLSearchParams | null): {
//   rows:number,page:number,search?:string
// } {
